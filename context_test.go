...
func TestContextWithCID_with_string(t *testing.T) {
	...
	tests := []struct {
		...
		wantValue              string
		wantErr                bool
		correlationIDGenerator func() (string, error) // override the cid generator for testing blank values
	}{
		{
			...
			wantValue:              "1",
			wantErr:                false,
			correlationIDGenerator: func() (string, error) { return uuid.Must(uuid.NewV4()).String(), nil },
		},
		{
